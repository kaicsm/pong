...
PongState::PongState(Game &game)
    : game(game), ball(Vector2{200, 200}, Vector2{200, 200}, 10.0f),
      paddle1(
          Rectangle{15.0f,
                    static_cast<float>(GetScreenHeight()) / 2.0f - 70.0f / 2,
                    20.0f, 70.0f},
          0),
      paddle2(
          Rectangle{static_cast<float>(GetScreenWidth() - 20.0f - 15.0f),
                    static_cast<float>(GetScreenHeight() / 2.0f - 70.0f / 2),
                    20, 70},
          0) {}
...
void PongState::handleInputs() {
  ...
  if (paddle1.bounds.y + paddle1.bounds.height > GetScreenHeight()) {
    paddle1.bounds.y =
        GetScreenHeight() - paddle1.bounds.height; // Corrige posição inferior
    paddle1.setSpeed(0);
  }
  ...
  if (paddle2.bounds.y + paddle2.bounds.height > GetScreenHeight()) {
    paddle2.bounds.y =
        GetScreenHeight() - paddle2.bounds.height; // Corrige posição inferior
    paddle2.setSpeed(0);
  }
...
void PongState::update(float deltaTime) {
  ...
  Rectangle screenBoundsBottom = {0, static_cast<float>(GetScreenHeight()),
                                  static_cast<float>(GetScreenWidth()), 1};
  ...
  if (ball.getPosition().x <= 0 || ball.getPosition().x >= GetScreenWidth()) {
    ball.reset(Vector2{(GetScreenWidth() - ball.getRadius()) / 2.0f,
                       (GetScreenHeight() - ball.getRadius()) / 2.0f});
  }
